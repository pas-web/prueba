...
def validar(R):
    ...
    fondo = R.get("fondo", {})
    fv = fondo.get("fuente")
    if fv and fv != "drone" and not os.path.exists(fv):
        fallos += err(f"Fondo armónico apunta a archivo inexistente: {fv}")
    return fallos
...
def lista_archivos(R):
    ...
    fv = R.get("fondo", {}).get("fuente")
    if fv and fv != "drone": archivos.add("./" + fv)
